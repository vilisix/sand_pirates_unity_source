    ...
    private void GenerateWave()
    {
        ...
        for (int x = 0; x <= Dimension; x++)
        {
            for (int z = 0; z <= Dimension; z++)
            {
                ...
                {
                    float timerF = Time.time;
                    if (Octaves[o].alternate)
                    {
                        var perl = Mathf.PerlinNoise((x * Octaves[o].scale.x) / Dimension, (z * Octaves[o].scale.y) / Dimension) * Mathf.PI * 2f;
                        y += Mathf.Cos(perl + Octaves[o].speed.magnitude * timerF) * Octaves[o].height;
                    }
                    else
                    {
                        var perl = Mathf.PerlinNoise((x * Octaves[o].scale.x + timerF * Octaves[o].speed.x) / Dimension, (z * Octaves[o].scale.y + timerF * Octaves[o].speed.y) / Dimension) - 0.5f;
